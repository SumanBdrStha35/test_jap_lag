...
class _TreeWithFallingLeavesState extends State<TreeWithFallingLeaves>
  ...
  Future<List<KanjiStep>> _loadKanjiSteps() async {
    try {
      ...
      return jsonMap.entries
          .map(
            (entry) =>
                KanjiStep.fromJson(int.tryParse(entry.key) ?? 0, entry.value),
          )
          .toList()
  ...
  List<Widget> _buildKanjiGrid() {
    ...
    while (currentIndex < _steps.length) {
      final itemsInRow = _calculateItemsForRow(
        rowIndex,
        _steps.length - currentIndex,
      );
  ...
  void _handleKanjiTap(KanjiStep item) {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => KanjiLessonPage(id: item.id, name: item.name),
      ),
    );
  }
    ...
    return Scaffold(
      body: Stack(
        children: [
          // Background
          Positioned.fill(
            child: Image.asset('assets/background_tree.jpg', fit: BoxFit.cover),
          ),
          ...
          // Kanji content
          if (!_isLoading) _KanjiContent(children: _buildKanjiGrid()),
        ],
      ),
    );
  }
...
class _LeafWidgetState extends State<_LeafWidget>
  ...
  void initState() {
    ...
    _controller = AnimationController(vsync: this, duration: fallDuration);

    // Sync animation to start time
    final elapsed =
        DateTime.now().millisecondsSinceEpoch - widget.leafData.startTime;
